...
Rename `prod.parameters.example` to `prod.parameters` and update the values. 

You need to provide the Alchemy and Moralis API keys in order to have access to their API.

To depoy the backend, run the following commands:

```
sam build
sam sam deploy --on-failure DELETE --parameter-overrides $(cat prod.parameters)
```
   * `--no-cached` guarantees that even if your dependencies gets updated, a new build will occur. Sam doesn't observe dependencies for changes. 
...
## What does it deploys? 
   ...
   * 1 API Gateway
   * 2 Lambdas to serve API requests
   * 4 Lambdas acting as Cognito Lambda triggers to manage authentication challenges
   * 1 Cognito User Pool to collect our identities
   * 1 Cognito Identity Pool to obtain AWS temporary credentials even for anonymous users so our API is not public
   * Some IAM Roles
...
There are 5 API calls in this example:
   ...
   * **/getFromLambda**: Get NFTs from eith Alchemy or Moralis through API Gateway Lambda proxy integrations (more logics, more customization, more cost, more latency) (`authenticated only`)
   * **/corsProxy**: Route to proxy calls to different domains. Useful to retrieve the NFTs' `.json` metadata files from hosts that are not CORS friendly. The dApp calls this route by appending a URL to the `url` query string parameter like this `/corsProxy?url=https://www.google.com`. 10MB max payload (API Gateway limitation) (`public facing`)
